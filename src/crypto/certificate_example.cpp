...
void demonstrateCertificateValidation() {
    ...
    try {
        ...
        // 创建有效的证书模板
        X509* validCert = NewCertificateTemplate("valid.example.com", now, validEndTime);
        if (validCert) {
            Certificate cert(validCert);
            std::cout << "有效证书验证结果: " << 
                (ValidateCertificate(cert, false) ? "通过" : "失败") << std::endl;
        }
        
        // 创建过期的证书模板  
        X509* expiredCert = NewCertificateTemplate("expired.example.com", now, expiredEndTime);
        if (expiredCert) {
            Certificate cert(expiredCert);
