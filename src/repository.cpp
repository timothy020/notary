...
//                         content = utils::Base64Decode(dataStr);
//                     }
//                 }
...
//         if (!content.empty()) {
//             changeData["data"] = utils::Base64Encode(content);
//         } else {
...
Repository::Repository(const GUN& gun, const std::string& trustDir, const std::string& serverURL)
    ...
    , cache_(std::make_shared<storage::FileSystemStorage>(trustDir, ".json"))
    ...
    try {
        ...
        if (remoteStore_) {
            for (const auto& [roleName, data] : updatedFiles) {
                err = remoteStore_->SetRemote(gun_.empty() ? "default" : gun_, roleName, data);
                if (!err.ok()) {
...
Result<std::vector<uint8_t>> Repository::resignMetadata(const std::vector<uint8_t>& metadata, const std::string& role) {
    try {
        // 解析元数据
        std::string metadataStr(metadata.begin(), metadata.end());
        json meta = json::parse(metadataStr);
        ...
        auto expiry = utils::getDefaultExpiry(roleName);
        ...
        // 获取私钥和角色名
        auto [privateKey, keyRole] = keyResult.value();
        if (!privateKey) {
            return Result<std::vector<uint8_t>>(Error("PrivateKey获取失败，无法签名"));
        }
        
        // 序列化元数据
        std::string metadataSignedStr = meta["signed"].dump();
        std::vector<uint8_t> metadataBytes(metadataSignedStr.begin(), metadataSignedStr.end());
        
        // 使用私钥签名
        std::vector<uint8_t> signature;
        try {
            signature = privateKey->Sign(metadataBytes);
        } catch (const std::exception& e) {
            return Result<std::vector<uint8_t>>(Error(std::string("Failed to sign metadata: ") + e.what()));
        }
        ...
        meta["signatures"].push_back({
            {"keyid", keys[0]},
            {"sig", utils::Base64Encode(signature)}
        });
...
Error Repository::initializeSnapshot() {
    try {
            ...
            {"signed", {
                ...
                {"expires", std::to_string(
                    std::chrono::duration_cast<std::chrono::seconds>(
                        utils::getDefaultExpiry(RoleName::SnapshotRole).time_since_epoch()).count())},
                {"meta", {}}
            }}
        };
        ...
        if (targetsResult.ok()) {
            ...
            std::string targetsStr(targetsResult.value().begin(), targetsResult.value().end());
            auto targetsHash = calculateHashes(targetsStr);
        ...
        return cache_->Set(SNAPSHOT_ROLE, snapshotData);
    } catch (const std::exception& e) {
